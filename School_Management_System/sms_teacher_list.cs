...
{
    public partial class sms_teacher_list : Form
    {
        sch_operation op = new sch_operation();
        string sql, items = "10";
        string status = "null", teacherid;
        public sms_teacher_list()
        {
            InitializeComponent();
        }

        private void sms_teacher_list_Load(object sender, EventArgs e)
        {
            refreshData();
        ...
        public void refreshData()
        {
            try
            {
                ...
                sql = "select top " + items + " * from sch_v_teacher order by Hired_Date";
        ...
        private void cboItems_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                if (items == "All")
                {
                    sql = "select * from sch_v_teacher order by Hired_Date";
                }
                else
                {
                    sql = "select top " + items + " * from sch_v_teacher order by Hired_Date";
        ...
        private void btnnew_teacher_Click(object sender, EventArgs e)
        {
            status = "new";
            sms_create_teacher teacher = new sms_create_teacher(status, teacherid);
            teacher.ShowDialog();
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                teacherid = dataGridView1.SelectedRows[0].Cells[0].Value + string.Empty;
            }
        }
